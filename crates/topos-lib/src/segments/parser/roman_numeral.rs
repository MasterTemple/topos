...
impl RomanNumerals {
    const CHARS: [char; 5] = ['i', 'v', 'x', 'l', 'c'];

    pub fn parser<'a>() -> impl Parser<'a, &'a str, u8> {
        any()
            // .filter(|c: &char| Self::CHARS.contains(&c.to_ascii_lowercase()))
            .filter(|c: &char| Self::value(*c).is_ok())
            .repeated()
            .at_least(1)
            .at_most(9) // Just to keep the parser from getting trolled
            .to_slice()
            // .try_map(|v, span| Ok(parse_roman_numeral(v)))
            .try_map(|v, span| {
                Self::convert(v).map_err(|e| {
                    dbg!(e);
                    EmptyErr::default()
                })
            })
    ...
    fn convert(s: &str) -> Result<u8, &'static str> {
        let err_msg = "Expected Roman numeral less than or equal to 255";
        let mut res: u8 = 0;
        let mut i = 0;

        while i < s.len() {
            // Get the value of the current symbol
            let s1 = Self::value(s.chars().nth(i).unwrap())?;

            // Compare with the next symbol if it exists
            if i + 1 < s.len() {
                let s2 = Self::value(s.chars().nth(i + 1).unwrap())?;

                // If current value is greater or equal, add it to result
                if s1 >= s2 {
                    res = res.checked_add(s1).ok_or(err_msg)?;
                    i += 1;
                } else {
                    // Else, add the difference and skip next symbol
                    res = res.checked_add(s2 - s1).ok_or(err_msg)?;
                    i += 2;
                }
            } else {
                res = res.checked_add(s1).ok_or(err_msg)?;
    ...
    fn value(ch: char) -> Result<u8, &'static str> {
        Ok(match ch {
            'I' | 'i' => 1,
            'V' | 'v' => 5,
            'X' | 'x' => 10,
            'L' | 'l' => 50,
            'C' | 'c' => 100,
            _ => Err("Invalid value, expected one of ['i', 'v', 'x', 'l', 'c']")?,
        })
    }
}
